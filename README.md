...
Hi. I'm a starter theme called `dana`. I'm a theme meant for hacking so don't use me as a Parent Theme. Instead try turning me into the next, most awesome, WordPress theme out there. That's what I'm here for.
...
`dana` requires the following dependencies:
...
To start using all the tools that come with `dana`  you need to install the necessary Node.js and Composer dependencies :
...
`dana` comes packed with CLI commands tailored for WordPress theme development :
...
4. Search for `dana` (in uppercase) to capture constants and replace with: `Nombre_proyecto`.
